...
function constraint_voltage_bounds_cc(pm::_PM.AbstractPowerModel, i::Int; nw::Int=pm.cnw)
    ...
    for l in 1:L
        ...
        # If the lhs summation is 0, don't add to the vector or we break the solver
        # as it doesn't like having 0's in the vector
        if lhs_summation == 0
            continue
        end
...
function constraint_flow_limits_cc(pm::_PM.AbstractPowerModel, l::Int; nw::Int=pm.cnw)
    ...
    for c in 1:C 
        ...
        for j in 1:L
            ...
            # If there is are no downstream nodes, don't add to the vector or we break the solver
            # as it doesn't like having 0's in the vector
            if lhs == 0
                continue
            end
